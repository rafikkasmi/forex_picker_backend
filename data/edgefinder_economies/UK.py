...
def GBP(inverse_inflation=True):
    #scoring of edge finder is build this way :
    ...
    gbp_cot=COTIndicator(market='BRITISH POUND STERLING - CHICAGO MERCANTILE EXCHANGE')
    gbp_cot.get_cot_data()
    gbp_cot.to_csv()
  
    
    gbp=[inflation,core_inflation,interest_rate,gdp,services_pmi,manufacturing_pmi,retail_sales,nfp,unemployment_rate]

    # gbp_instance=EdgeFinderInstance(market='gbp',indicators=gbp,cot=gbp_cot,seasonality=gbp_seasonality,trend_reading=gbp_trend)
    return gbp,gbp_cot
