...
def NZD(inverse_inflation=True):
    #scoring of edge finder is build this way :
    #get inflation data (cpi, core cpi)
    inflation = EconomicIndicator('inflation-rate', 
                                    [0.01,0,-0.01],
                                    [1,  0, -1],score_based_on="mom",country=country,datasource='fxempire',frequency='Q',is_negative=inverse_inflation,name="inflation")
    inflation.get_data()
    ...
    retail_sales = EconomicIndicator('retail-sales-mom', 
                                    [0.01,0,-0.01],
                                    [1,  0, -1],score_based_on="mom",country=country,datasource='fxempire',frequency='Q',name="retail-sales")
    ...
    nfp =  EconomicIndicator('employment-change', 
                                    [0.01,0,-0.01],
                                    [1,  0, -1],score_based_on="mom",country=country,datasource='fxempire',frequency='Q',name="employment-change")
    nfp.get_data()
    nfp.calculate_score()
    ...
    unemployment_rate =  EconomicIndicator('unemployment-rate', 
                                    [0.01,0,-0.01],
                                    [1,  0, -1],score_based_on="mom",country=country,datasource='fxempire',frequency='Q',is_negative=True,name="unemployment-rate")
    unemployment_rate.get_data()
    unemployment_rate.calculate_score()


    nzd_cot=COTIndicator(market='NZ DOLLAR - CHICAGO MERCANTILE EXCHANGE')
    nzd_cot.get_cot_data()
    
    nzd=[inflation,core_inflation,interest_rate,gdp,services_pmi,manufacturing_pmi,retail_sales,nfp,unemployment_rate]

    # nzd_instance=EdgeFinderInstance(market='nzd',indicators=nzd,cot=nzd_cot,seasonality=nzd_seasonality,trend_reading=nzd_trend)
    return nzd,nzd_cot
