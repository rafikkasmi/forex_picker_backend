...
def CAD(inverse_inflation=True):
    #scoring of edge finder is build this way :
    ...
    cad_cot=COTIndicator(market='CANADIAN DOLLAR - CHICAGO MERCANTILE EXCHANGE')
    cad_cot.get_cot_data()
    cad_cot.to_csv()
   
    
    cad=[inflation,core_inflation,interest_rate,gdp,services_pmi,manufacturing_pmi,retail_sales,nfp,unemployment_rate]

    # cad_instance=EdgeFinderInstance(market='cad',indicators=cad,cot=cad_cot,seasonality=cad_seasonality,trend_reading=cad_trend)
    return cad,cad_cot
